...
class AgentProcessInput:
    ...
    def __post_init__(self):
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', self.name):
            errors.append("Agent name can only contain letters, numbers, underscores, and hyphens")
        ...
        if self.child_type not in config_manager.agents:
            errors.append(f"Invalid runner type '{self.child_type}'. Must be one of: {list(config_manager.agents.keys())}")
