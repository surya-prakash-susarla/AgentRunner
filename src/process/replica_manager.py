...
from process.exceptions import (
    MaxChildrenExceededError, ChildAgentNotFoundError, ChildAgentExistsError,
    ChildAgentNotRunningError, ChildAgentTimeoutError, ChildAgentOperationError
)
import logging
...
class ReplicaManager:
    ...
    def create_child(self, name: str, runner: GeminiRunner) -> None:
        ...
        if len(self.children) >= self.max_children:
            logger.warning("Cannot create child '%s': maximum children limit (%d) reached", 
                         name, self.max_children)
    ...
    def ask_child(self, name: str, question: str, timeout: Optional[float] = None) -> str:
        ...
        try:
            ...
            logger.error("Error getting response from child agent '%s': %s", name, str(e))
